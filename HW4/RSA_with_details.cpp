...
void env(string modulus, string message, string exp, int modulus_length){
    ...
    try{
        // // n
        // HexDecoder decoder_n;
        // decoder_n.Put((const byte*)modulus.data(), modulus.size());
        // decoder_n.MessageEnd();
        // Integer n;
        // n.Decode(decoder_n, decoder_n.MaxRetrievable());
        Integer n = convert(modulus);

        // // e
        // HexDecoder decoder_e;
        // decoder_e.Put((const byte*)exp.data(), exp.size());
        // decoder_e.MessageEnd();
        // Integer e;
        // e.Decode(decoder_e, decoder_e.MaxRetrievable());
        ...
        Integer temp = Integer((const byte*)message.data(), message.size());
        if (temp > publicKey.MaxImage()) return;
...
void dec(string modulus, string cipherText, string dec_exp, int modulus_length){
    ...
    try{
        ...
        Integer e = convert(exp);
            ...
            // d
        // HexDecoder decoder_d;
        // decoder_d.Put((const byte*)dec_exp.data(), dec_exp.size());
        // decoder_d.MessageEnd();
        // Integer d;
        // d.Decode(decoder_d, decoder_d.MaxRetrievable());
