...
const int num_leftShift[16] = {2, 2, 2, 1,
							   2, 2, 2, 1,
							   2, 2, 2, 1,
							   2, 2, 2, 1};  // number of bits to shift for each iteration
...
vector<int> S(int k, vector<int> slice_R)
{
	int num, flag = 0;
	if(k == 0) k = 7, flag = 1;
	else if (k == 7 && !flag) k = 0, flag = 1;
	
	if(k != 1) num = s_box[k][slice_R[0] * 2 + slice_R[5]][slice_R[1] * 8 + slice_R[2] * 4 + slice_R[3] * 2 + slice_R[4]];
	else num = (2 * slice_R[0] + 3 * slice_R[1] + 5 * slice_R[2] + 7 * slice_R[3] + 11 * slice_R[4] + 13 * slice_R[5]) % 16;
	vector<int> rev(4);
	for (int i = 3; i >= 0; i--)
	{
		rev[i] = num % 2;
...
void solve(vector<int> mainKey, vector<int> plainText)
{
	...
	// Step 4: Get the final L_16 and R_16,
...
signed main()
{
	...
	while (T--)
	{
		...
		string line;
		getline(cin, line);
		text = line.substr(0, 8);
		key = line.substr(9);
//		cout << "text: " << text << " key: " << key << "\n";
